...
The job key is derived from (source hash, parser version, normalizer
version) per spec 12.4: re-running an identical job returns the recorded
result and performs no writes; bumping either version creates a new derived
document version without mutating the source. Corpus versions are published
atomically — a single transaction flips the active pointer, and a partial
unique index guarantees at most one active version even under races.
"""
...
from fel_workers.ingestion.parser import PARSER_VERSION, ParseError, parse_filing
...
def job_key(source_hash: str, parser_version: str, normalizer_version: str) -> str:
    """Deterministic job identity per spec 12.4."""
    seed = f"{source_hash}|{parser_version}|{normalizer_version}"
...
def _prior_canonical_facts(
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        rows = cur.execute(
            ...
            ORDER BY ff.fact_key, d.published_at DESC
            """,
...
def ingest_filing(
    ...
    *,
    ...
    Idempotent: an identical (source, parser, normalizer) job is a no-op
    returning the recorded result. Malformed sources are quarantined with an
    actionable diagnostic instead of failing the job.
    """
    content_hash, _ = content_address(raw)
    key = job_key(content_hash, PARSER_VERSION, NORMALIZER_VERSION)
    with conn.transaction():
        with conn.cursor(row_factory=dict_row) as run_cur:
            existing = run_cur.execute(
                "SELECT job_key, document_id, document_version_id, result"
        ...
        version_id = str(
            uuid.uuid5(
                uuid.NAMESPACE_URL,
                f"https://financial-evidence-lab.dev/version|{content_hash}"
                f"|{PARSER_VERSION}|{NORMALIZER_VERSION}",
            )
        )
        try:
            parsed = parse_filing(raw, id_seed=version_id)
        ...
        conn.execute(
            ...
            (version_id, stored.document_id, PARSER_VERSION, NORMALIZER_VERSION),
        )
        ...
        conn.execute(
            ...
            " normalizer_version, status, document_id, document_version_id, result)"
...
def _record_quarantine(
    ...
    *,
    ...
    conn.execute(
        "INSERT INTO ingestion_runs (job_key, source_hash, parser_version,"
        " normalizer_version, status, document_id, result)"
        " VALUES (%s, %s, %s, %s, 'quarantined', %s, %s)",
        (
            key,
            content_hash,
            PARSER_VERSION,
...
def publish_corpus_version(conn: psycopg.Connection[Any], corpus_version_id: str) -> None:
    ...
    Both updates happen in one transaction; readers see either the old or the
    new active pointer, never zero or two. The partial unique index
    ``corpus_versions_single_active`` fails any racing second publisher.
    """
    with conn.transaction():
        conn.execute(
            "UPDATE corpus_versions SET is_active = false, status = 'superseded'" " WHERE is_active"
        )
        cur = conn.execute(
        ...
        if cur.rowcount != 1:
            raise ValueError(f"corpus version {corpus_version_id} does not exist or is not a draft")
