...
format: ## Auto-format all sources
	pnpm run format
	$(PY)/black apps workers evals packages/providers

format-check: ## Verify formatting without writing
	pnpm run format:check
	$(PY)/black --check apps workers evals packages/providers

lint: ## Lint all sources
	pnpm run lint
	$(PY)/ruff check apps workers evals packages/providers

typecheck: ## Run static type checks
	pnpm run typecheck
	$(PY)/mypy apps/api/app workers/src evals/graders packages/providers/fel_providers
...
security: ## Run static + dependency security scans
	$(PY)/bandit -q -r apps workers evals packages/providers -c pyproject.toml
