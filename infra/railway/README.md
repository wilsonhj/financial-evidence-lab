...
- `api.json` — FastAPI service (`uvicorn`, `/health` healthcheck). Its
  `startCommand` runs the migration applier (`scripts/db/migrate.py`,
  ADR-0014) before `uvicorn`; the applier takes an advisory lock and is a
  no-op when the schema is current, so several replicas starting together
  are safe. Railway also documents a `deploy.preDeployCommand` field for
  this purpose; the start-command prefix is used instead because the field
  could not be verified against the live schema from CI, and the prefix
  works on every builder.
- `worker.json` — job-queue consumer (`python -m fel_workers run`). It
  refuses to start on a stale schema (`migrate.py --check`) and serves
  `GET /health` on `$PORT` (`FEL_WORKER_HEALTH_PORT`, see
  `workers/src/fel_workers/health.py`) so `healthcheckPath` restarts a
  wedged consumer whose heartbeat has stalled. `ON_FAILURE` remains the
