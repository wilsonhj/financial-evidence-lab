...
import { generateContractTypes } from "./generate";
...
describe("contract schemas", () => {
  ...
  it("generated strict payloads accept real KPI, edit and correction values without metadata", () => {
    const payload: components["schemas"]["ExtractionPayload"] = {
    ...
    const evidence: components["schemas"]["EvidenceEdge"][] = [
      {
    ...
    const correction: components["schemas"]["CorrectionCommand"] = {
      reason: "Source checked",
      payload,
      evidence,
    };
    const review: components["schemas"]["ReviewCommand"] = {
      action: "edit",
      extraction_ids: ["00000000-0000-4000-8000-000000000001"],
      expected_versions: { "00000000-0000-4000-8000-000000000001": 1 },
      reason: "Source checked",
      patch: [{ extraction_id: "00000000-0000-4000-8000-000000000001", payload, evidence }],
    };
    expect(payload).toEqual(load("fixtures/extraction-payload.json"));
    expect(correction.payload).toBe(payload);
    expect(ajv.getSchema(SCHEMA_IDS.extractionReviewCommand)!(review)).toBe(true);
    const { value, ...missingValue } = payload;
    expect(value).toBe("100");
    // @ts-expect-error A required financial value remains required.
    const incomplete: components["schemas"]["ExtractionPayload"] = missingValue;
    // @ts-expect-error Monetary values remain decimal strings.
    const wrongType: components["schemas"]["ExtractionPayload"] = { ...payload, value: 100 };
...
describe("contract version identity (VERSIONING.md)", () => {
  it("openapi, package, and CONTRACT_VERSION agree on the 0.9.0 minor bump", () => {
    ...
    expect(CONTRACT_VERSION).toBe("0.9.0");
...
describe("generated client drift (check:generated in-suite)", () => {
  it("src/generated/api.ts is exactly the approved generator output — hand edits fail", async () => {
    const apiPath = join(here, "src/generated/api.ts");
    const expected = await generateContractTypes();
