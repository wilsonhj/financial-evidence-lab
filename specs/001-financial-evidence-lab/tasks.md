...
- [ ] `T0112` Ingest the first 20 benchmark issuers and record corpus-quality metrics. — not delivered by the `M1-CORPUS-QA` merge (PR #86): #56 is reopened for the live 20-issuer ingest, which needs provisioned credentials; the live-cutover package tracked in #177 owns it once registered.
...
- [ ] `T0215` Run the reference-corpus retrieval performance suite (Atlas performance testing deferred post-MVP). — left unchecked at the 2026-09-03 reconciliation: `packages/retrieval/ACCEPTANCE.md` lists exact-vs-HNSW p95/latency characterization beyond the smoke gate as a known deferral, so the acceptance evidence is not present (follow-ups #132, #137).
...
- [x] `T0411` Parse user-authored arithmetic formulas into a bounded typed AST, extract dependencies, and evaluate with Decimal/unit/period validation and immutable versioned provenance (§8.5, `FR-MOD-002`; ADR-0018). Lead verified after PR #262 merge at `dff71f0`: independent engine tests, legacy hashes and final CI passed; no hosted performance claim.
- [x] `T0412` Evaluate explicitly declared cyclic formula groups with deterministic bounded fixed-point iteration, explicit seeds/tolerances, nonconvergence failure and reproducible lineage (§8.5; ADR-0018). Lead verified after PR #262 merge at `dff71f0`: independent engine tests, legacy hashes and final CI passed; no hosted performance claim.
