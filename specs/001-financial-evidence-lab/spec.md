...
- One monorepo deployed to Railway as separate Next.js web, FastAPI API, and Python worker processes.
- Supabase provides managed Postgres/pgvector, Auth, row-level security, and Storage.
- Local development runs Next.js and FastAPI directly against a hosted Supabase development project; Docker is not required.
- Sentry and structured JSON logs provide initial application observability.
- GitHub Actions provides CI and deployment gates.
- Platform configuration files and SQL migrations are the initial infrastructure-as-code boundary; Terraform/Pulumi are deferred.
