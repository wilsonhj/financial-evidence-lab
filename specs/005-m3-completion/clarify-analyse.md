...
| **Q-2** | #132 directs `claude-opus-4-8` against ADR-0002's accepted "OpenAI for generation" | **Draft ADR-0012 (Proposed)** for the substitution, split by role, and explicit that the required benchmark evidence does not yet exist. ADR-0002 remains controlling, so OpenAI is lawful now; ADR-0012 is required only before choosing Anthropic |
...
| **A-2** | HIGH | #132 proposes substituting `claude-opus-4-8` for the OpenAI generation provider approved by ADR-0002, but the substitution lacks an approved ADR and benchmark evidence | **OpenAI remains the lawful baseline.** ADR-0012 is drafted (Proposed) and gates only an Anthropic substitution; it does not gate `M3-304` as written |
...
| **A-5** | MEDIUM | The `0006` entry and #62 claim the same two path globs, so they can never run concurrently — #62 is already registered; only the `0006` entry is missing from the queue | **Modelled** in `spec.md` §4.3 and §6.1; the contention note is written into the registration PR's entry comment |
...
**Result:** Q-1 through Q-4 are resolved and reflected in `spec.md` and `plan.md`. A-4 through A-8 are recorded with owners and routes. **A-1 (credential and adapter readiness) gates M3 completion. A-2 gates only a provider substitution**; the OpenAI baseline remains available under ADR-0002.
...
- **Simplicity and provider isolation:** satisfied by retaining ADR-0002's OpenAI baseline. An Anthropic substitution remains unavailable until ADR-0012 is supported by benchmark evidence and accepted.

**Gate result: FAILS, one of five.** The credential/adapter-readiness failure is
a pre-existing repository condition this document surfaces and assigns an owner
to. The Governance clause is unqualified — *"a failed mandatory gate blocks
merge"* — so merging this specification requires an integration-lead waiver
recorded on its pull request, as `specs/004-mvp-completion/` also required.
