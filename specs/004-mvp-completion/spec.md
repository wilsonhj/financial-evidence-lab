...
**Where the handoff documents and this file disagree, this file's numbers were measured and the handoff documents' were not.** `STATUS.md` and `README.md` are being reconciled by PR #172. `tasks.md` has not been reconciled at all (§4.2).
...
| `M4-MODEL-CALC` | #63 | T0401–T0403, T0409, T0410 | 1 | **All deps merged; contends with nothing dispatchable.** Inherits `blocked`, so dispatching means *adding* a `status:` key |
...
Plus **one package this spec proposes registering** (§5.1): `RELEASE-LIVE-CUTOVER`, which `M5-AUDIT-RELEASE` must then depend on.
...
| Registering a new package is itself a shared-path change | `workstreams.yaml` is in `shared_paths` | `contract-change` + authorization record per #141 |
