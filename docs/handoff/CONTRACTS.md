...
- **v0.3.0** (2026-07-16, issue #100, ADR-0006): additive observable retrieval surface — `POST /v1/workspaces/{workspaceId}/queries` (optional `CreateQuery.index_version_id`), query snapshot + rerun, typed SSE `GET /v1/retrieval-runs/{runId}/events` (`RetrievalEvent` / `retrieval-event/v1`), trace read, evidence feedback; required `QueryPlan.index_version_id`; schemas/fixtures for `query-plan`, `retrieval-event`, `retrieval-trace`, `evidence-feedback`; migration `db/migrations/0003_retrieval_core.sql` (shared index artifacts SELECT-only; tenant query/trace/claim RLS). Leaves `0004_extraction_core.sql` for #101.
