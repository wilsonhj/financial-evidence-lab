...
- **v0.5.0** (issue #191, PR #232): additive 402 cost-limit and 429 rate-limit responses on existing retrieval operations, including Retry-After; generated HTTP types updated.
- **v0.6.0** (issue #193, PR #241, accepted ADR-0015; serialized after PR #232’s v0.5.0): additive `claims-output/v1` schema + fixture registered in `SCHEMA_IDS` — the reader's structured-generation contract, consumed by `StructuredClaimGenerator` and produced by the deterministic mock model. `VERSIONING.md` classifies new schemas as additive even without new HTTP endpoints: OpenAPI `info.version`, `CONTRACT_VERSION`, and the package version therefore advance together. Generated HTTP types remain unchanged. Numeric claims independently assert finite decimal-string value, integer scale, and nullable unit/period; null denotes unknown metadata and cannot inherit evidence metadata. Optional members remain required-and-nullable for strict structured output. No database migration or live provider adapter is introduced.
- **v0.7.0** (2026-09-10, #191, ADR-0021): explicit list page mode (`limit`, `cursor`, `order`) preserves array and query snapshot bodies, with `X-FEL-Next-Cursor`, `X-FEL-Previous-Cursor`, and `X-FEL-Page-Limit`. Legacy reads are complete up to 50 rows or return 409 `PAGINATION_REQUIRED`. Reader 1.1.0 adds optional `sibling_page` coverage, target-only reads, and bounded sibling pages; individual oversized evidence returns 413 `READER_TOO_LARGE`. Event history has a separate bounded page contract, complete trace overflow returns `TRACE_TOO_LARGE`, and version references resolve at most 200 requested IDs. Continuations are unsigned untrusted scope-bound data; every request reapplies authorization and visibility. High water is not an MVCC snapshot.

- **v0.8.0** (#278, ADR-0024): typed atomic extraction review, complete conflict membership/preconditions, immutable approval validation context, nullable uncalibrated confidence and bounded history. Migration 0011 and the compatible worker preserve legacy NULL conflict groups and support opted-in run/v1 occurrences without inherited adjudication. New schemas use the documented offline reference aliases. #61/#135 API/UI implementation remains dependent work; no live-provider or hosted gate is certified.
