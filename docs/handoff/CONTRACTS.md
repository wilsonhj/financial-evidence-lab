...
- **Pending** (issue #193, ADR-0012): additive `claims-output/v1` schema + fixture registered in `SCHEMA_IDS` — the reader's structured-generation contract, consumed by `StructuredClaimGenerator` and produced by the deterministic mock model. Schemas-only: `openapi/openapi.yaml` and `CONTRACT_VERSION` are untouched (no HTTP surface change; the version bump belongs to the next OpenAPI-owning change). Optional members are modelled required-and-nullable so the same object can be sent to a strict structured-output provider unchanged.
