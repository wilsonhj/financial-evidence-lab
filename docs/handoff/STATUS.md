...
Only item 1 is `ready` in `workstreams.yaml`. Items 2 and 3 are `blocked` there and are listed here as the next candidates with their real gates, not as dispatchable work — do not start them on the strength of this heading.
...
- **#155** — [M3-EXTRACTION] `normalize/dimensions.py` and `normalize/currency.py` have no importers and duplicate live `payload.py` logic. Open, `agent-task`, a direct #145 follow-up; absent from the #145 follow-up set recorded in both files (#146/#62/#153/#154/#157/#158).
