...
- Next dispatch: **M4-MODEL-CALC (#63) is `status: ready`** — dependency satisfied (`M3-EXTRACTION-CORE` @ `61058e4`), `packages/calculation-engine/**` overlaps nothing in flight, credentials mock-only; this reconciliation **adds** the key (it previously inherited `defaults.status: blocked`). **M3-REVIEW (#61)** still carries an explicit `status: blocked` that was **not** flipped: ADR-0011 is being ratified on PR #175 (Accepted on that PR); SSE still waits until migration 0006 lands via #157; #146 was ruled Option 1 (terminal runs final) on 2026-08-29 but is not yet implemented — still a producer gate. M3-CONFIDENCE-GATE (#62) is **not** unblocked — it `depends_on: [M3-REVIEW]`, so it waits on #61 merging, not on #60.
- Integration lead ruled ADR-0011 over ADR-0009; ratification is PR #175 (Accepted on that PR). Do not expose SSE until migration 0006 lands via #157. ADR-0009 (`docs/decisions/ADR-0009-checkpoint-payload-in-event-stream.md`) landed with #145 under shared `docs/decisions/**` and remains **Status: Proposed** until #175 supersedes it. PR #156 corrected four factually false statements in it, but did **not** ratify it.
...
**Open PRs** as of 2026-08-30: #172 (this ledger reconcile), #173 (004-mvp-completion), #175 (ADR-0011 ratification). Re-run `gh pr list --state open` rather than trusting this list. #174 is merged, not open. Open issue count measured at 31 after the #171/#134 closes; re-run `gh issue list --state open --limit 100 --json number --jq length` to confirm.
...
**M4-MODEL-CALC (#63) is `status: ready` in `workstreams.yaml`.** EVALS-REPORT-RENDER merged via PR #164 and stays `merged`. M3-REVIEW stays `blocked` (not flipped). Re-run `gh pr list --state open` before dispatching.

1. **M4-MODEL-CALC (#63)** — **`ready`**. `depends_on: [M3-EXTRACTION-CORE]`, merged at `61058e4`; `allowed_paths` are `packages/calculation-engine/**` alone, which overlap no other package and nothing in flight among open PRs #172/#173/#175. Branch `agent/m4-model-calc`. #174 is on main @ `a4bb356`; credentials mock-only; dispatchable now. This reconciliation **adds** the `status: ready` key (it previously inherited `defaults.status: blocked`).
2. **M3-REVIEW (#61)** — dependency satisfied since #145 merged, so its `status: blocked` is **no longer a statement about dependencies**; the comment on that entry in `workstreams.yaml` now says so. Branch `agent/m3-review` (not yet created); paths `apps/web/**`, `apps/api/**`. It carries an **explicit** `status: blocked` key, so dispatching it means flipping that key rather than adding one. **What actually holds it:** ADR-0011 is being ratified on PR #175 (Accepted on that PR); SSE still waits until migration 0006 lands via #157; #146 was ruled Option 1 (terminal runs final) on 2026-08-29 but is not yet implemented — still a producer gate. Not flipped by this reconciliation.
...
4. **ADR-0011 over ADR-0009.** Integration lead ruled ADR-0011; ratification is PR #175 (Accepted on that PR). Do not expose SSE until migration 0006 lands via #157. **#158** (verify the restored checkpoint against a hash) is independent of the fork and needs no contract change.
...
- **#134** — M2-OBSERVATORY: run Playwright E2E in CI (closes criterion 6 browser-E2E). **CLOSED** (PR #131).
...
- PR **#131** — Observatory Playwright E2E in fixture mode (**merged** @ `bc6a2a3`; #59 criterion 6). Closed **#134**.
