...
| Keyboard graph/table, visible source/confidence/validation/conflict/history, safe drafts and navigation |24 fixture Chromium tests plus production HTTP/queue/Next/Chromium acceptance;291 terminal-selection/comparison repair,294 standalone patch parsing,297 stale expected-version/winner pruning |Preserve these final-head required CI checks. This bounded audit did not rerun an independent accessibility study or claim full hosted UI coverage. |
...
3. `apps/api/tests/extraction/test_sse_lifecycle.py`: real local HTTP socket against current route; drain initial authorized batch and establish waiting_review before the negative action. Use a separate same-org viewer whose membership has never authored a policy, run, review or approval, prove its stream read access, then delete only that membership. Do not delete the fixture owner: restrictive author foreign keys would reject setup before testing revocation. Owner→viewer alone is **not revocation**, because viewers may read; commit a later event and require no new event payload reaches that revoked client, with bounded stream termination. Synchronize the next fetch authorization after the revocation commit. Once HTTP 200 headers are sent, a closed transfer/protocol error is an acceptable termination; do not require a new HTTP 403 or graceful EOF. Use a valid other-run/other-tenant event ID for resume and require initial404. Disconnect a live waiting stream and verify its DB checkout/transaction is released, with no worker thread/connection leak and subsequent authorized requests succeeding. Compare actual checked-out connections/active transactions to the pre-stream baseline, not only a mocked close method. Persistent pool connections and threadpool workers need not disappear. An oversized persisted payload should fail explicitly (413 before streaming for an initial oversized event), never truncate/skip it or expose content. Keep invalid safe-integer tests already present. A heartbeat check may use a controlled monotonic clock seam while keeping real socket/PG; do not wait minutes or alter production intervals.
...
## Verified role and correction replay basis

Accepted ADR0024 lines147–154 assigns owner/editor all eight actions, reviewer accept/edit/reject/merge/correct, viewer no mutations, and all four roles read access. Current routes_runs.py:89–100 projects this same matrix; runs.py:30/295/352 and review.py:95/corrections.py:56 enforce it. Therefore own-visible valid requests from viewer must return403 for all eight actions; reviewer must return403 only for create/cancel/rerun. Use a legitimate foreign-tenant **owner** for hidden-resource404 tests so an earlier role denial cannot mask tenant isolation. Foreign resources behind workspace routes must use the foreign workspace path; unscoped resource endpoints use actual foreign resource IDs.

Same-key concurrent corrections are required by backend plan:89–91/109/139: authorization precedes receipt lock, canonical request includes actor/resource/action/body/If-Match, and same request replays original status/body/ETag/Location before stale preconditions. Current corrections.py:67–69 calls receipts.begin before resource discovery; receipts.py:29–32 takes transaction advisory lock before receipt lookup. Thus use the **same actor, exact body, original ETag and same key** concurrently, expect201/201 with identical bytes/ETag/Location and exactly one appended version, audit and receipt. Different keys against the same head remain201/412. Changed request under the same key is409 and is already a separate contract boundary, not the same-key concurrency case.
