...
- [ ] `T0002` Add local Docker Compose for web, API, worker, PostgreSQL/pgvector, object storage, and queue.
- [ ] `T0003` Define OpenAPI and JSON-schema versioning rules and generate the TypeScript client.
- [ ] `T0004` Implement OIDC login, organizations, membership, and owner/editor/reviewer/viewer roles (`FR-WRK-001`–`004`).
...
- [ ] `T0009` Provision development/staging infrastructure as code and test backup restoration.
- [ ] `T0010` Publish provider interfaces for LLMs, embeddings, rerankers, object storage, and market data.
...
- [ ] `T0108` Implement the first BYO market-data adapter with adjusted prices, volume, and corporate actions (`FR-ING-008`).
