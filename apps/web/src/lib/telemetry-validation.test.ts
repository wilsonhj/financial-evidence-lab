...
describe("redact", () => {
  ...
  it("redacts an api-key style key=value pair", () => {
    // The value is deliberately low-entropy and obviously not a key. It still
    // exercises API_KEY_RE, which matches on the `api_key=` prefix and a
    // non-space run, not on the value's shape. An `sk-`-style literal here
    // trips gitleaks' generic-api-key rule (entropy 4.25) and fails the secret
    // scan on a test fixture -- the repo fixed the same false positive in
    // af4679e by changing the literal rather than suppressing the finding.
    expect(redact("failed with api_key=EXAMPLE-NOT-A-REAL-KEY")).toBe(
